...
A chamada de IA em si passa por _chamar_ia (app/services/ai.py) -- o
Adaptador de provedor de IA (Gemini/OpenAI), compartilhado com
tutor_service e gerar_quiz/gerar_cards. Este arquivo não sabe qual dos
dois provedores está por trás; só monta os prompts e passa TUTOR_MODEL
(mesmo modelo rápido/barato do Tutor geral) pro caso de o provedor ser
o Gemini.
"""
...
from app.services.ai import _chamar_ia
...
def explicar_erro(
    ...
    texto = _chamar_ia(prompt, user_id, db, instrucao_sistema=PERSONA_TUTOR, model=TUTOR_MODEL)
...
def refinar_explicacao(
    ...
    texto_novo = _chamar_ia(prompt, user_id, db, instrucao_sistema=PERSONA_TUTOR, model=TUTOR_MODEL)
